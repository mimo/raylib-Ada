...
begin
    ...
    while not should_close loop
        ...
        if input.is_key_pressed (KEY_P) then
            if (view.projection = CAMERA_PERSPECTIVE) then
                ...
                view.fovy := 20.0;
                rcamera.Camera_Yaw
                   (view'unchecked_access, (-135.0 * PI / 180.0), True);
                rcamera.Camera_Pitch
                   (view'unchecked_access,
                    (-45.0 * PI / 180.0),
                    True,
                    True,
                    False);
                   ...
                   Vector3'
                      (x =>
                          (if input.is_key_down (KEY_W)
                              or input.is_key_down (KEY_UP)
                           then 0.1
                           elsif input.is_key_down (KEY_S)
                              or input.is_key_down (KEY_DOWN)
                           then -0.1
                           else 0.0),
                       y =>
                          (if input.is_key_down (KEY_D)
                              or input.is_key_down (KEY_RIGHT)
                           then 0.1
                           elsif input.is_key_down (KEY_A)
                              or input.is_key_down (KEY_LEFT)
                           then -0.1
                           else 0.0),
                       z => 0.0),
                rotation =>
        ...
        text.draw
           ("- Move keys: W, A, S, D, Space, Left-Ctrl", 15, 30, 10, BLACK);
        ...
        text.draw
           ("- Mode: "
            & (if use_camera_pro then "CAMERA_PRO" else cam_mode'Img),
            600,
            30,
            10,
            BLACK);
        ...
        text.draw
           ("- Position: ("
            & fp (view.position.x)'Img
            & ","
            & fp (view.position.y)'Img
            & ","
            & fp (view.position.z)'Img
            & ")",
            600,
            60,
            10,
        ...
        text.draw
           ("- Target: ("
            & fp (view.target.x)'Img
            & ","
            & fp (view.target.y)'Img
            & ","
            & fp (view.target.z)'Img
            & ")",
            600,
            75,
            10,
