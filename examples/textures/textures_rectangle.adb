...
procedure textures_rectangle is
    ...
    frames_speed   : Integer :=
       8; -- Number of spritesheet frames shown by second
begin
    raylib.window.init
       (screen_width,
        screen_height,
    ...
    while not raylib.window.should_close loop
        ...
        if input.is_key_pressed (KEY_RIGHT) then
            frames_speed := frames_speed + 1;
        elsif input.is_key_pressed (KEY_LEFT) then
            frames_speed := frames_speed - 1;
        end if;

        if frames_speed > MAX_FRAME_SPEED then
            frames_speed := MAX_FRAME_SPEED;
        elsif frames_speed < MIN_FRAME_SPEED then
